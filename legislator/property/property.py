...
def get_type(df):
    ...
    for row in rows:
        if pd.notnull(row) and isinstance(row, basestring):
            match = re.search(u'(?:公職人員)?(\S*財產申報表)', row)
            if match:
...
categories = [u'土地', u'建物', u'船舶', u'汽車', u'航空器', u'現金', u'存款', u'股票', u'債券', u'基金受益憑證', u'其他有價證券', u'具有相當價值之財產', u'保險', u'債權', u'債務', u'事業投資', u'備註', u'此致']
for f in files:
    print f
    ...
    writer = pd.ExcelWriter('output/%s_%s_%s_%s.xlsx' % (os.path.splitext(os.path.basename(f))[0], name, date, title), engine='xlsxwriter')
