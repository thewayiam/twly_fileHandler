...
import psycopg2,win32com,glob,os,re
    ...
    if bill_id:
        ...
        for row in range(startrow , doc.Tables[0].Rows.Count+1):
            if doc.Tables[0].Columns.Count == 3:
                ...
                AddBillDetail(bill_id, article, doc.Tables[0].Cell(row, 2).Range.Text, doc.Tables[0].Cell(row, 1).Range.Text, doc.Tables[0].Cell(row, 3).Range.Text)
