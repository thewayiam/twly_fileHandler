...
def InsertUpdateProposal(uid, sitting_id, proposal_seq, content):
    c.execute('''UPDATE proposal_proposal
            SET content = %s
            WHERE uid = %s''', (content, uid)
    )
    c.execute('''INSERT into proposal_proposal(uid, sitting_id, proposal_seq, content, hits, likes, dislikes) 
            SELECT %s, %s, %s, %s, 0, 0, 0
            WHERE NOT EXISTS (SELECT 1 FROM proposal_proposal WHERE uid = %s )''', (uid, sitting_id, proposal_seq, content, uid))  

def InsertUpdateLegislatorProposal(legislator_id, proposal_id, priproposer):
    c.execute('''UPDATE proposal_legislator_proposal
            SET priproposer = %s
            WHERE legislator_id = %s AND proposal_id = %s''', (priproposer, legislator_id, proposal_id)
    )
    c.execute('''INSERT into proposal_legislator_proposal(legislator_id, proposal_id, priproposer)
            SELECT %s, %s, %s
            WHERE NOT EXISTS (SELECT 1 FROM proposal_legislator_proposal WHERE legislator_id = %s AND proposal_id = %s)''', (legislator_id, proposal_id, priproposer, legislator_id, proposal_id)
    )  

def GetSession(text):
    ms, me, uid = re.search(u'立法院(第(?P<ad>[\d]+)屆第(?P<session>[\d]+)會期(第(?P<temptimes>[\d]+)次臨時會)?(?P<committee>[\W\s]{2,39})(兩|三|四|五|六|七|八)?委員會[\s]*第(?P<times>[\d]+)次(全體委員|聯席)會議)議事錄',text), re.search(u"[\s]+散[\s]{0,4}會",text), None
    if ms:
